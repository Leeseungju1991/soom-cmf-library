...
<TouchableOpacity activeOpacity={0.11} onPress={()=> navigation.navigate("Wishlist1",{item})}>
  ...
  useEffect(() => {
    ...
    return () => {
      // WebView 인스턴스를 정리하는 로직 추가
      // 예를 들어, WebView 컴포넌트의 ref를 사용하여 웹뷰를 정리할 수 있습니다.
      // WebView 컴포넌트를 ref로 참조하고 ref를 사용하여 WebView를 정리합니다.
      // 아래 예제에서는 webViewRef로 WebView 컴포넌트를 참조하고 정리하는 방법을 보여줍니다.
      if (webViewRef) {
  ...
  return (
    ...
    <TouchableOpacity onPress={()=>navigation.goBack()} >
      ...
      <CaretLeft  size={27} color={'#484848'}style={{ marginLeft : 20, color : '#484848', fontFamily: "Pretendard", fontWeight: "500", fontSize : 17, fontStyle: "normal",  textAlign: "left",}}/>
       ...
       <View style={{marginTop : 0}}>
        ...
        <Text style={{color : '#E3E3E3' ,fontSize : 28}}>{"1"}</Text>
          </ImageBackground>
        </ImageBackground>
   ...
   <Modal
    ...
    >
            ...
            style={{
              width: 250,
              height: 200, // 높이를 200으로 설정하여 이미지를 잘라냅니다.
              overflow: 'hidden', // 이미지를 잘라내도록 설정합니다.
              alignItems: 'center',
              justifyContent: 'center',
              marginTop : '40%'
            }}
